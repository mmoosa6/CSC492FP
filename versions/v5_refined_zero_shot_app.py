...
st.markdown(
    "This prototype uses a transformer-based NLP model "
...
if st.button("Analyze Email"):
    ...
    if email_body.strip() == "":
        ...
        labels = [
            "malicious phishing email attempting credential theft",
            "legitimate safe business communication"
        ]

        result = classifier(email_text, labels)

        raw_prediction = result["labels"][0]
        confidence = result["scores"][0]

        if "phishing" in raw_prediction:
            prediction = "phishing"
        else:
